...
def main():
    ...
    adj = get_adjacency_jsonG('Generate_data/Json_G/json_G.json').to(device)
    ...
    model.train()
    #
    # data.train_mask = torch.tensor([True if i%2==0 else False for i in range(727)])#torch.tensor([[True,False],[True,False],[True,False]])
    # data.train_mask2 = torch.tensor([[True,True] for i in range(727)])
    #
    train_y = data.y[data.train_mask]  # 训练标签data.y[data.train_mask]
    for epoch in range(epochs):
        ...
        val_acc = test(data.train_mask, model, x, data.y, adj)#test(data.val_mask, model, x, data.y, adj)  # 计算当前模型在验证集上的准确率
    ...
    test_acc = test(data.train_mask, model, x, data.y, adj)#test(data.test_mask, model, x, data.y, adj)  # 测试集正确率
